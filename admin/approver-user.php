...
if(isset($fld_username)){
	$fld_approverlevel = $_POST['fld_approverlevel'];
...
//echo "Stored in: {$destination}";
		  ...
		  $q = mysqli_query($con,"insert into tbl_approveruser(fld_fullname,fld_approverlevel,fld_desigcode,fld_profilepic,fld_emailadd,fld_password,fld_accountstatus)VALUES('$fld_fullname','$fld_approverlevel','$fld_desigcode','$basename','$fld_username','$new_pass','$fldstatus')");
		  $err = 2;
	  }
	}
	else{
...
//echo "Stored in: {$destination}";
		  ...
		  $q = mysqli_query($con,"UPDATE tbl_approveruser SET fld_fullname='$fld_fullname',fld_approverlevel='$fld_approverlevel',fld_desigcode='$fld_desigcode',fld_profilepic='$basename',fld_password='$new_pass' where fld_id='$fld_idtxt'");
		  $err = 3;
		  $fldoldemail="None";
		  $fldidd="None";
		}
...
//echo "Stored in: {$destination}";
		  ...
		  $q = mysqli_query($con,"UPDATE tbl_approveruser SET fld_emailadd='$fld_username',fld_fullname='$fld_fullname',fld_approverlevel='$fld_approverlevel',fld_desigcode='$fld_desigcode',fld_profilepic='$basename',fld_password='$new_pass' where fld_id='fld_idtxt'");
		  $err = 3;
		  $fldoldemail="None";
		  $fldidd="None";
		}
...
if($deptId!=''){
	...
	if($deptact=="edit"){
	$qdept = mysqli_query($con,"select * from tbl_approveruser");
	while($rdept=mysqli_fetch_array($qdept)){
		$theid = md5($rdept['fld_id']);
		if($theid==$deptId){
			
			$fldidd = $rdept['fld_id'];
			$fld_deptcod = $rdept['fld_desigcode'];
			$fldfullname = $rdept['fld_fullname'];
			$fldusername = $rdept['fld_emailadd'];
			$fldoldemail = $rdept['fld_emailadd'];
			$fldapproverlevel = "<option value='".$rdept['fld_approverlevel']."'>".$rdept['fld_approverlevel']."</option>";
	...
	while($rdept=mysqli_fetch_array($qdept)){
		if(md5($rdept['fld_id'])==$deptId){
			...
			$qept = mysqli_query($con,"Delete from tbl_approveruser where fld_id='$fld_d'");
	...
	while($rdept=mysqli_fetch_array($qdept)){
		if(md5($rdept['fld_id'])==$deptId){
			...
			$qt = mysqli_query($con,"UPDATE tbl_approveruser SET fld_accountstatus='$stat' where fld_id='$fld_id'");
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-6">
                                <ul class="breadcome-menu">
                                    ...
                                    <li><span class="bread-blod">Approver User</span>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    ...
                    <div class="sparkline13-list shadow-reset">
                        <div class="sparkline13-hd">
                            <div class="main-sparkline13-hd">
                                <h1><i class="fa big-icon fa-users"></i> Approver Users</h1>
                            </div>
                        </div>
								...
								<form method="POST" id="form1" action="<?php echo $_SERVER['PHP_SELF']; ?>" enctype="multipart/form-data">
                                    ...
                                    <div class="col-lg-4">
                                        <div class="basic-login-inner"><br><br>
											...
											<input type="text" name="fld_idtxt" value="<?php echo $fldidd;?>">
                                                ...
                                                <div class="form-group-inner">
                                                    <div class="row">
                                                        <label style="float:left;padding-left:15px">Approver Level</label>
                                                        <div class="col-lg-12">
														<select class="form-control" name="fld_approverlevel" required>
														  <?php
														   echo $fldapproverlevel;
                                    ...
                                    <div class="col-lg-8">
                                        <table id="table" data-toggle="table" data-pagination="true" data-search="true"
											...
											<?php 
											$ctr=1;
											 $q = mysqli_query($con,"select * from tbl_approveruser order by fld_id");
											 while($r=mysqli_fetch_array($q)){
												 $afld_id=$r['fld_id'];
											  echo "<tr>";	 
											?>
											 <td><?php echo $ctr;?></td>
											 <td><?php echo $r['fld_approverlevel'];?></td>
...
Swal.fire({
  ...
  if (result.isConfirmed) {
    window.location.href = 'approver-user.php?action='+ status +'&deptId='+userId;
  }
})
...
<script type='text/javascript'>
	...
	if(<?php echo $err; ?> == 2){
			Swal.fire(
  'Saved',
  'The New Approver User has been Added!',
  'success'
);
	...
	if(<?php echo $err; ?> == 3){
		Swal.fire(
  'Updated!',
  'The Approver User has been Updated!',
  'success'
);
}
	if(<?php echo $err; ?> == 4){
		Swal.fire(
  'Deleted!',
  'The Approver User has been Deleted!',
  'success'
);
