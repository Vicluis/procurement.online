		 ...
		 while($r=mysqli_fetch_array($q)){
			 if(md5($r['fld_id'])==$deptId){
				 ...
				 if($levelnya=="Level 2"){
					...
					if($apctrup>0){
						$x = mysqli_query($con,"UPDATE tbl_lpofiles SET fld_approvertwo='$st',fld_approvertwoid='$apid' where fld_id='$theid'");
						$err = 1;
					}
					else{
						$err =2;
					 ...
					 $apctrup = mysqli_num_rows($x);
					if($apctrup>0){
						$x = mysqli_query($con,"UPDATE tbl_lpofiles SET fld_approverthree='$st',fld_approverthreeid='$apid' where fld_id='$theid'");
						$err = 1;
					}
					else{
						$err =2;
					 ...
					 $apctrup = mysqli_num_rows($x);
					if($apctrup>0){
						$x = mysqli_query($con,"UPDATE tbl_lpofiles SET fld_approverfour='$st',fld_approverfourid='$apid' where fld_id='$theid'");
						$err = 1;
					}
					else{
						$err =2;
					 ...
					 $x = mysqli_query($con,"UPDATE tbl_lpofiles SET fld_approverone='$st',fld_approveroneid='$apid' where fld_id='$theid'");
					 $err = 1;
				 }
			 }
		 }
                    ...
                    <div class="sparkline13-list shadow-reset">
                        <div class="sparkline13-hd">
                            <div class="main-sparkline13-hd">
                                <h1><i class="fa big-icon fa-eye"></i> For Review LPOs</h1>
                            </div>
                        </div>
	...
	while($r=mysqli_fetch_array($q)){
		...
		echo "<tr>";
											...
											<td><div class="btn-group project-list-ad">
                                                            <a href="javascript:;" onclick="changestatus('<?php echo md5($afld_id); ?>', 'pending');return false;" class="btn btn-white btn-xs"><i
                                                                    class="fa fa-pencil"></i> To Pending</a>
                                                        </div></td>
											<?php 
											echo "</tr>";
											$ctr++;
...
Swal.fire({
  title: 'Put to Pending?',
  text: 'Are You sure You want to put this on Pending',
  icon: 'question',
  ...
  if (result.isConfirmed) {
    window.location.href = 'for-review.php?action='+ status +'&deptId='+userId;
  }
})
...
<script type='text/javascript'>
	...
	if(<?php echo $err; ?> == 1){
			Swal.fire(
  'Updated',
  'The LPO has been Moved to Pending!',
  'success'
);
